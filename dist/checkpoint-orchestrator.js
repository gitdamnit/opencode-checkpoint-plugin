...
function generateHandoffMarkdown(data) {
    ...
    const currentPlan = sanitizedPlan.length > 0
        ? sanitizedPlan.map((s, i) => `${i + 1}. ${s}`).join("\n")
        : "None recorded.";
    const completedSteps = sanitizedCompleted.length > 0
        ? sanitizedCompleted.map((s) => `- [x] ${s}`).join("\n")
        : "None.";
    const nextSteps = sanitizedNext.length > 0
        ? sanitizedNext.map((s) => `- [ ] ${s}`).join("\n")
        : "None.";
    const blockers = sanitizedBlockers.length > 0
        ? sanitizedBlockers.map((b) => `- ⚠️ ${b}`).join("\n")
        : "None.";
    const filesTouched = sanitizedFiles.length > 0
        ? sanitizedFiles.map((f) => `- 📄 ${f}`).join("\n")
        : "None recorded.";
    const notes = sanitizedNotes.length > 0
    ...
    return {
        tool: {
            checkpoint_save: tool({
                description: "Save or update persistent task state for later recovery or handoff. Fields are deep-merged so partial updates don't clobber existing data. Optionally provide snapshotName to tag this save for later reference.",
                args: {
            ...
            checkpoint_list: tool({
                description: "List all snapshots in checkpoint history, newest first. Returns name, savedAt, task title and status for each entry. Use checkpoint_load_snapshot to inspect full data, checkpoint_restore_snapshot to restore.",
                args: {},
                async execute() {
                    const history = await loadHistory();
                    ...
                    return JSON.stringify({ snapshots: list, count: history.snapshots.length }, null, 2);
                },
            }),
            checkpoint_load_snapshot: tool({
                description: "Load full data for a specific named snapshot from history. Read-only — does NOT overwrite live checkpoint.json. Use checkpoint_restore_snapshot to promote a snapshot to live state.",
                args: {
            ...
            checkpoint_restore_snapshot: tool({
                description: "Restore a named snapshot as the live checkpoint state. Before overwriting, the current live state is automatically saved as a snapshot named 'pre-restore-{timestamp}' so you can undo.",
                args: {
                    name: tool.schema.string(),
                },
                async execute(args) {
                    ...
                    const entry = history.snapshots.find((s) => s.name === args.name);
                    if (!entry) {
                        return JSON.stringify({ error: "Snapshot not found", name: args.name, available: history.snapshots.map((s) => s.name) }, null, 2);
                    }
                    if (liveState) {
                        ...
                        return JSON.stringify({ restored: true, name: args.name, undoSnapshot: preRestoreName }, null, 2);
                    }
                    await save(entry.data);
                    return JSON.stringify({ restored: true, name: args.name, undoSnapshot: null }, null, 2);
                },
            }),
        },
                ...
                if (existing) {
                    ...
                    await ctx.client.app.log({
                        body: {
                            service: "checkpoint",
                            level: "info",
                            message: "Auto-snapshot on session.compacted (existing state re-saved)",
                        },
                    });
                }
                else {
                    await save({
                        ...
                        summary: "Session compacted without prior checkpoint_save call. State synthesized automatically.",
                        notes: [
                    ...
                    await ctx.client.app.log({
                        body: {
                            service: "checkpoint",
                            level: "warn",
                            message: "Auto-snapshot on session.compacted (synthesized — no prior save found)",
                        },
                    });
                }
            }
